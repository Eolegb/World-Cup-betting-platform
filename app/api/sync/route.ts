...
export async function POST(req: Request) {
  ...
  if (syncing) {
    return NextResponse.json({ ok: true, message: "Sync already in progress" }, { status: 200 })
  }
  ...
  try {
    ...
    // 2. Sync live fixtures for live matches
    ...
    // 3. Sync events for live matches
    ...
    results.push(`Fetched odds for ${odds.length} matches (used on next bet page load)`)
