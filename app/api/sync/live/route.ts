...
import { fetchLiveFixtures, fetchMatchDetail, type FootballDataMatch } from "@/lib/providers"
  ...
  try {
    ...
    for (const f of fixtures) {
      ...
      const [existing] = await db
        .select({ id: match.id, oldStatus: match.status })
        .from(match)
      ...
      await db
        .update(match)
        .set({ status, homeScore, awayScore, elapsed: isFinished ? 90 : null, lastSyncedAt: new Date() })
        .where(eq(match.id, existing.id))
      updated++

      // Fetch goals from match detail
      if (!isFinished) {
      ...
      // Settle bets if match just finished
      if (isFinished && existing.oldStatus !== "finished") {
        const settled = await settleMatchByExternalId(existing.id)
        betsSettled += settled
      }
    }
...
async function settleMatchByExternalId(matchId: number): Promise<number> {
